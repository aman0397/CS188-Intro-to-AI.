...
#        min_food=100
        ...
        for x in range(newFood.width):
            for y in range(newFood.height):
                if oldFood[x][y] :
                    min_food=manhattanDistance((x,y),newPos)
                    if min_food == 0 :
                        score+=200
                    else:
                        if min_food<food_min :
                            food_min=min_food
                            flag=True
        if flag :
            score+=(1/food_min**1.5)+random.randint(0,1)
        for capsule in currentGameState.getCapsules():
            if manhattanDistance(capsule,newPos) == 0 :
                score+=500
            else :
                score+=(1/manhattanDistance(capsule,newPos)**2)
...
class MinimaxAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        final_action=""
        max=-10000
        for agent in range(1,getNumAgents())
            for action in legalMoves:
                d=self.minValue(gameState.generateSuccessor(agent,action), self.depth)
            if d > max :
                max = d	
                final_action=action
        return final_action
...
#                 for action in legalMoves :
#                     d=self.maxValue(gameState.generateSuccessor(agent,action), depth-1)
#                     if d < v :
#                         v=d
#                 if v < min :
#                     min=v
...
#                 legalMoves=gameState.getLegalActions(agent)
#                 for action in legalMoves :
#                     d=self.minValue(gameState.generateSuccessor(agent,action), depth-1)
#                     if d > v :
#                         v=d
#                 if max < v :
#                     max=v
